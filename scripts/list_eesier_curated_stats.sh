...
mkdir ${SCRIPT_DIR}/../testdata/benchmarks/stats
for e_count in ${evidence_counts[@]}; do
    for o in "meet" "max" "join"; do
        mkdir -p ${SCRIPT_DIR}/../testdata/benchmarks/stats/curated/evid${e_count}/${o}
        # Iterating through the thresholds like this is annoying and redundant, but
        # I do not want to change this ATM.
        for t in 1 100; do
            for prj in "zlib" "pidgin" "littlefs"; do
              bazel run //cli:main -- --db-name eesier_curated_evid${e_count}_${o} eesi ListStatisticsDatabase \
                  --bitcode-uri ${BASE_BC_DIR}${prj}-reg2mem.bc \
                  --ground-truth-path ${GT_DIR}/${prj}-gt.txt \
                  --confidence-threshold ${t} \
                  > ${SCRIPT_DIR}/../testdata/benchmarks/stats/curated/evid${e_count}/${o}/${prj}-${t}-stats.txt
            done
        done
    done
done
