...
void errorpropagation(Module &Mod, string error_only_path,
                      string input_specs_path, string error_codes_path);
void fullpropagation(Module &Mod, string error_only_path);
void specs(Module &Mod, string error_only_path, string input_specs_path, string error_codes_path);
...
int main(int argc, char **argv) {
  ...
  desc.add_options()("help", "produce help message")
      ...
      ("errorcodes", po::value<string>(), "Path to input specs list file")
  ...
  string error_codes_path;
  if (varmap.count("errorcodes")) {
    error_codes_path = varmap["errorcodes"].as<string>();
  }
  ...
  if (command == "specs") {
    specs(*Mod, error_only_path, input_specs_path, error_codes_path);
    ...
    errorpropagation(*Mod, error_only_path, input_specs_path, error_codes_path);
...
void specs(Module &Mod, string error_only_path, string input_specs_path, string error_codes_path) {
  ...
  ErrorBlocks *error_blocks =
      new ErrorBlocks(error_only_path, input_specs_path, error_codes_path, &Mod);
  ReturnedValues *returned_values = new ReturnedValues();
  PM.add(return_propagation);
  PM.add(return_constraints);
  PM.add(error_blocks);
