...
ErrorBlocks::ErrorBlocks(string error_only_path, string input_specs_path, string error_codes_path, Module *mod)
    : ModulePass(ID) {
  readErrorOnlyFile(error_only_path);
  readInputSpecsFile(input_specs_path, mod);
  if (!error_codes_path.empty()) {
    readErrorCodesFile(error_codes_path);
  }
}
...
void ErrorBlocks::readErrorCodesFile(string error_codes_path) {
  string line;

  ifstream error_codes_file(error_codes_path);
  while (getline(error_codes_file, line)) {
    vector<string> fields;
    boost::split(fields, line, boost::is_any_of(" "));
    if (fields[1].find("0x") != std::string::npos) {
      error_codes.insert(stoi(fields[1], 0, 0));
    }  else {
      error_codes.insert(stoi(fields[1]));
    }
  }
  if (error_codes.size() == 0) {
    LOG(WARNING) << "EMPTY ERROR-CODES SET!";
  }
}

void ErrorBlocks::readInputSpecsFile(string input_specs_path, Module *mod) {
  string line;
  ...
  while (getline(input_specs_file, line)) {
    ...
    if (!mod->getFunction(fname)) {
	continue;
    }
    LOG(INFO) << fname << " " << fields[1];
