...
struct ErrorBlocks : public llvm::ModulePass {
  ...
  ErrorBlocks(std::string error_only_path, std::string input_specs_path, std::string error_codes_path, llvm::Module *mod);
...
private:
  ...
  void readInputSpecsFile(std::string input_specs_path, llvm::Module *mod);
  void readErrorCodesFile(std::string error_codes_path);
  ...
  // A block is an error block if it calls an error-only function
  // or if the constraint on that block satisfies the error
  // specification of a function
  std::unordered_map<llvm::BasicBlock *, bool> error_states;
