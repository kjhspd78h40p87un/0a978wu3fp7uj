...
def get_specifications_all(database, service_configuration_handler,
    ...
    for uri in list(cli.bitcode.db.read_uris(database)):
        ...
        embedding_id = None
        if synonym_configuration_handler.use():
            embedding_uri = cli.embedding.db.read_embedding_uri(
                database, bitcode_id_handle.id)

            # If the user passes --use-embedding, but no embedding
            # information is stored in database.
            if not embedding_uri:
                log.info("No embedding information for "
                         f"{cli.common.uri.to_str(uri)} in MongoDB. To infer "
                         "specifications without using embeddings, "
                         "remove the --use-embeddings flag")
                partial_completion = True
                continue
            ...
            embedding_id.authority = service_configuration_handler.\
                embedding_config.get_full_address()
...
def get_predicted_specifications_uri(database,
                                     service_configuration_handler,
                                     top_k, bitcode_uri):
    """Gets the predicted specifications according to the registered model.

    Gets the top-k most similar functions for every function according to
    the registered embedding in the MongoDB. The most similar functions'
    specifications that are inferred and part of the specifications response
    in the database are then compared to the function of interest (what we
    called most similar on) where the ground truth is the provided EESI
    results, if the specifications match, we consider that to be a true
    positive, whereas if the specifications are different that is a false
    positive. However, if the specification is not inferred, that is simply
    a false negative and doesn't affect our precision, but only recall. This
    is primarily intended to run on the specifications inferred only by EESI
    and specifications results inferred from EESIER.

    Args:
        database: pymongo database object.
        service_configuration_handler: ServicesConfigurationHandler with
            services configured for: embedding
        top_k: The top_k most similar functions in the embedding for each
            function label.
        bitcode_uri: The bitcode URI for the project of interest that is
            registered in the database.
    """

    # Changing into proto message.
    bitcode_uri = cli.common.uri.parse(bitcode_uri)

    bitcode_id = cli.bitcode.db.read_id_for_uri(database, bitcode_uri)
    if not bitcode_id:
        log.info(f"{cli.common.uri.to_str(bitcode_uri)} "
    ...
    # We can't calculate anything if we don't have an embedding.
    if not embedding_uri:
        log.info("No embedding information for "
                 f"{cli.common.uri.to_str(bitcode_uri)} in MongoDB. Skipping!")
        return None

    function_specifications = cli.eesi.db.read_specifications_response_dict(
        database, bitcode_id)

    # We also can't calculate anything if we don't have stored results.
    if not function_specifications:
        log.info(f"==No specifications for {cli.common.uri.to_str(bitcode_uri)}"
                 " found in supplied database!==")
        return None

    # Registering the embedding with the service.
    embedding_id = cli.embedding.rpc.register_embedding(
        service_configuration_handler.get_embedding_stub(),
        proto.embedding_pb2.RegisterEmbeddingRequest(uri=embedding_uri))
    embedding_id.authority = service_configuration_handler.\
    ...
    fp = 0
    fn = 0
    for function_name, lattice_element in \
            function_specifications.items():
        get_most_similar_request = \
            proto.embedding_pb2.GetMostSimilarRequest(
                embedding_id=embedding_id,
                label=proto.get_graph_pb2.Label(label=function_name),
                top_k=top_k)

        function_synonyms = []
        try:
            specifications_most_similar_response = \
                cli.embedding.rpc.get_most_similar(
                    service_configuration_handler.get_embedding_stub(),
                    get_most_similar_request)
            function_synonyms = [
                x.label for x in
                specifications_most_similar_response.labels]
        # Catching the instance where a function name is not part of the
        # embedding's vocabulary.
        # TODO (): This is generic and bad, should be replaced
        # with a more elegant/specific solution.
        except Exception:
            continue

        for function_synonym_name in function_synonyms:
            if function_synonym_name not in function_specifications:
                fn += 1
            elif function_specifications[function_synonym_name] is \
                 lattice_element:
                tp += 1
            else:
                fp += 1
    ...
    log.info("="*30)
    log.info(f"Stats for {cli.common.uri.to_str(bitcode_uri)}")
    log.info(f"TP: {tp}")
    log.info(f"FP: {fp}")
    log.info(f"FN: {fn}")
...
def get_specifications_uri(database,
    ...
    if synonym_configuration_handler.use():
        ...
        # information is stored in database.
        if not embedding_uri:
            log.info(f"No embedding information for {bitcode_uri} in MongoDB."
                     " To infer specification without using embeddings,"
                     " remove the --use-embeddings flag.")
            cli.common.log.log_finished("GetSpecificationsUri", False)
            return
        ...
        embedding_id = cli.embedding.rpc.register_embedding(
            service_configuration_handler.get_embedding_stub(),
            proto.embedding_pb2.RegisterEmbeddingRequest(uri=embedding_uri))
        embedding_id.authority = \
            service_configuration_handler.embedding_config.get_full_address()
    ...
    print(applicable_initial_specifications)
...
def list_specifications_table(database, bitcode_uri_filter,
    ...
    print(colored(f"{'Bitcode ID (last 8)':<20} {'File name:':<30} "
    ...
    for bitcode_id, bitcode_uri in sorted(id_file_dict.items(), key=lambda x: x[1]):
        ...
        print(f"{bitcode_id[-8:]:<20} {os.path.basename(bitcode_uri):<30} "
...
def list_statistics_database(bitcode_uri, ground_truth_file, database, t):
    ...
    for specification in get_specifications_response:
        ...
        # FIXME(): I don't know what I was thinking before, but
    ...
    if total_third_party:
        third_party_recall = 100 * (third_party_inferred / total_third_party)
    delta_precision = 0.0
    ...
    print(f"3rd partyPrecision: {third_party_precision:.2f}%")
    print(f"3rd partyRecall: {third_party_recall:.2f}%")
